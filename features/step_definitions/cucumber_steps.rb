...
Given /^a standard Cucumber project directory structure in "([^\"]*)"$/ do |name|
  @current_dir = working_dir
  in_current_dir do
    FileUtils.rm_rf name if File.directory?(name)
    FileUtils.mkdir_p "#{name}"
    FileUtils.mkdir_p "#{name}/features"
    FileUtils.mkdir_p "#{name}/features/support"
    FileUtils.mkdir "#{name}/features/step_definitions"
  end
end
...
When /^I (?:have )?run rake (.*)$/ do |rake_opts|
  run "LIMITED_RED=on rake #{rake_opts} --trace"
end
